...
<head>
    ...
    <title><?php echo $title; ?> - <?php echo __('belediye_rehberi'); ?></title>
    ...
    <style>
        ...
        .business-header {
            height: 250px; background-size: cover; background-position: center; border-radius: 0 0 30px 30px;
...
<div id="app">
    <header class="header">
        <a href="district.php?slug=<?php echo $business['district_slug']; ?>" class="home-link">
    ...
    <main class="section animate-in">
            ...
            <?php endif; ?>
134: 
135:             <?php if ($business['order_enabled'] && !empty($business['order_link'])): ?>
136:                 <div style="margin-top: 15px;">
137:                     <a href="<?php echo htmlspecialchars($business['order_link']); ?>" target="_blank" class="btn btn-order" style="background: linear-gradient(45deg, #ff5722, #ff9800); color: #fff; font-weight: 700; width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px; padding: 15px; border-radius: 12px; text-decoration: none; box-shadow: 0 4px 15px rgba(255, 87, 34, 0.3);">
138:                         <i class="fa-solid fa-shopping-basket"></i> <?php echo __('order_now_btn') ?? 'Sipariş Ver'; ?>
139:                     </a>
140:                 </div>
141:             <?php endif; ?>
142: 
        ...
        if ($has_wh):
            ...
            if ($day_open) {
                if ($close_min < $open_min) {
                    ...
                    // Normal çalışma (Örn: 09:00 - 22:00)
                    $is_open = ($now_min >= $open_min && $now_min < $close_min);
                }
            }
            if (!$is_open) {
                // Bugün açılış yok (veya saat tutmadı) ama dün gece açılıp bugün sabaha kadar devam eden bir shiftte miyiz?
        ...
        <div class="card animate-in" style="margin-top: 20px;">
            <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px; margin-bottom:12px;">
                <h3 style="margin:0;"><i class="fa-solid fa-clock" style="color:var(--secondary);"></i> <?php echo __('working_hours'); ?></h3>
                <?php if ($is_open): ?>
                <span class="status-pill pill-open">
                    <span class="dot-live dot-open"></span> <?php echo __('currently_open'); ?>
                </span>
                <?php else: ?>
                <span class="status-pill pill-closed">
                    <span class="dot-live dot-closed"></span>
                    <?php echo $day_open ? __('closed_today_off') : __('closed_today'); ?>
                </span>
                <?php endif; ?>
            </div>
            ...
            <div style="font-size:0.85rem; color:rgba(255,255,255,0.75); margin-bottom:12px;">
                ...
                <?php echo __('today'); ?>: <strong style="color:#fff;"><?php echo $wh_data['open'] . ' – ' . $wh_data['close']; ?></strong>
            </div>
            <?php endif; ?>
            ...
            <table class="week-table">
                ...
                <tr class="<?php echo $is_today ? 'today-row' : ''; ?>">
                    <td class="day-name">
                        ...
                        <?php echo __(['sunday','monday','tuesday','wednesday','thursday','friday','saturday'][$di]); ?>
                        <?php if ($is_today): ?></strong> <small style="color:var(--secondary);">(<?php echo __('today'); ?>)</small><?php endif; ?>
                    </td>
                    <td style="text-align:right;">
                        <?php if ($is_day_open): ?>
                            <span class="tag-open"><?php echo $wh_data['open'] . ' – ' . $wh_data['close']; ?></span>
                        <?php else: ?>
                            <span class="tag-closed-day"><?php echo __('closed_today'); ?></span>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endfor; ?>
        ...
        <div class="card animate-in" style="margin-top: 20px;">
            ...
            <?php foreach ($products as $p): ?>
                <div style="display: flex; gap: 15px; padding: 15px 0; border-bottom: 1px solid var(--glass-bg);">
                    <?php if ($p['image_path']): ?>
                        <img src="<?php echo $p['image_path']; ?>" style="width: 70px; height: 70px; border-radius: 10px; object-fit: cover;">
                    <?php endif; ?>
                    <div style="flex: 1;">
        ...
        if (!$is_excluded) {
            ...
            echo '<div style="margin-top: 20px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 20px;">
                <button id="checkin-btn" class="btn btn-primary" onclick="app.handleCheckIn(' . $business['id'] . ', \'business\')" style="background: linear-gradient(45deg, #00c9ff, #92fe9d); color: #000; font-weight: 700; width: 100%;">
                    <i class="fa-solid fa-location-crosshairs"></i> ' . __('check_in_btn') . '
                </button>
            </div>';
        }
        ?>
