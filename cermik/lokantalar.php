...
<body data-page-context="<?php echo $title; ?>" data-district-slug="cermik">
...
<div id="app">
    ...
    <main class="section">
        <div id="business-list" class="menu-list">
            ...
            // Türkiye saati (UTC+3) - date_default_timezone_set yerine güvenilir hesaplama
            $now_ts  = time() + (3 * 3600);
            ...
            if ($businesses) {
                foreach ($businesses as $b) {
                    ...
                    if ($has_hours) {
                        if ($is_open) {
                            $badge = "<span class='status-badge status-open'><i class='fa-solid fa-circle' style='font-size:0.5rem;'></i> ". __('open_badge') ."</span>";
                        } else {
                            $badge = "<span class='status-badge status-closed'><i class='fa-solid fa-circle' style='font-size:0.5rem;'></i> ". __('closed_badge') ."</span>";
                        }
                    ...
                    $closed_overlay = (!$is_open && $has_hours)
                        ? "<div class='closed-overlay'><div class='closed-stamp'><i class='fa-solid fa-lock'></i> ". __('currently_closed') ."</div></div>"
                        : '';
