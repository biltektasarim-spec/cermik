...
<body data-page-context="Profil" data-district-slug="cermik">
...
<div id="app">
    ...
    <div class="profile-main animate-in">
        <div class="tabs-nav">
            <button class="tab-btn active" onclick="switchProfileTab('checkins', this)"><?php echo __('my_checkins'); ?></button>
        ...
        <div id="tab-checkins" class="tab-panel active">
            <?php if ($checkins): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p style="text-align: center; padding: 20px; opacity: 0.5;"><?php echo __('no_checkin_yet'); ?></p>
            <?php endif; ?>
        </div>
        ...
        <div class="card" style="margin-top: 15px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 8px;">
                <a href="javascript:void(0)" onclick="showPolicy('kvkk')" style="color: var(--text-secondary); text-decoration: none; font-size: 0.8rem; padding: 8px; background: rgba(255,255,255,0.03); border-radius: 8px;"><i class="fa-solid fa-file-contract"></i> <?php echo __('kvkk_text_link'); ?></a>
                <a href="javascript:void(0)" onclick="showPolicy('cookie')" style="color: var(--text-secondary); text-decoration: none; font-size: 0.8rem; padding: 8px; background: rgba(255,255,255,0.03); border-radius: 8px;"><i class="fa-solid fa-cookie-bite"></i> <?php echo __('cookie_policy_link'); ?></a>
            </div>
        </div>
        ...
        <div id="policy-modal" class="modal">
            <div class="modal-content animate-in" style="max-height: 80vh; overflow-y: auto;">
                <div class="modal-close" onclick="document.getElementById('policy-modal').classList.remove('active')">
                    <i class="fa-solid fa-times"></i>
                </div>
                <h2 id="policy-title">Politika</h2>
                <div id="policy-content" style="line-height: 1.6; color: var(--text-primary); margin-top: 15px;"></div>
            </div>
        </div>
    </div>
</div>
...
function showPolicy(type) {
    ...
    if (type === 'kvkk') {
        ...
        contentEl.innerHTML = document.getElementById('kvkk-content-data').innerHTML || '<?php echo __('no_info_entered'); ?>';
    } else if (type === 'cookie') {
        titleEl.textContent = '<?php echo __('cookie_policy_link'); ?>';
        contentEl.innerHTML = document.getElementById('cookie-content-data').innerHTML || '<?php echo __('no_info_entered'); ?>';
    }
...
function uploadAvatar(input) {
    if (input.files && input.files[0]) {
        ...
        if (file.size > 5 * 1024 * 1024) {
            app.showToast('<?php echo __('file_too_large'); ?>', 'error');
            return;
        }
        ...
        const isSubDirJS = <?php echo $is_sub_dir ? 'true' : 'false'; ?>;
        ...
        .then(res => {
            if(res.status === 'success') {
                const imgPrefixJS = isSubDirJS ? '../' : '';
                document.getElementById('avatar-preview').src = imgPrefixJS + res.image_path + '?v=' + Date.now();
                app.showToast('<?php echo __('profile_updated'); ?>', 'success');
            } else {
        ...
        .catch(err => {
            app.showToast('<?php echo __('error_occured'); ?>', 'error');
            document.getElementById('avatar-preview').style.opacity = '1';
        });
    }
}
