...
// Settings
$settings = get_settings($pdo, $user['district_id'] ?? 0);
?>
<!DOCTYPE html>
<html lang="tr">
<head>
    ...
    <link rel="stylesheet" href="<?php echo $img_prefix; ?>assets/css/style.css">
    <style>
        html, body { 
            height: 100dvh; 
            margin: 0; 
...
<body data-page-context="Profil" data-district-slug="<?php echo $user['district_id'] == 1 ? 'cermik' : 'cungus'; ?>">
...
<div id="app">
    ...
    <div class="profile-header animate-in">
        ...
        <!-- Android Native Google Giriş Butonu (Sadece Uygulama İçinde Görünür) -->
        <div id="android-profile-btn" class="android-only" style="margin-top: 15px; justify-content: center;">
            <button type="button" onclick="handleAndroidGoogleLogin()" class="btn-google" style="max-width: 250px;">
                <i class="fa-brands fa-google"></i> <span>Google Hesabını Bağla</span>
            </button>
        </div>
    </div>
    ...
    <div class="profile-main animate-in">
        <div class="tabs-nav">
            <button class="tab-btn active" onclick="switchProfileTab('checkins', this)">Ziyaretlerim</button>
        ...
        <div id="tab-checkins" class="tab-panel active">
            <?php if ($checkins): ?>
                <?php foreach ($checkins as $c): ?>
                    <div class="checkin-item">
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            ...
                            <span class="badge-status status-<?php echo $c['status']; ?>">
                                <?php echo $c['status'] === 'APPROVED' ? 'Onaylandı' : ($c['status'] === 'PENDING' ? 'Onay Bekliyor' : 'Reddedildi'); ?>
                            </span>
                        </div>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p style="text-align: center; padding: 20px; opacity: 0.5;">Henüz hiç mekan ziyareti yapmadınız.</p>
            <?php endif; ?>
        </div>
...
function uploadAvatar(input) {
    if (input.files && input.files[0]) {
        ...
        .catch(err => {
            app.showToast('Sunucu bağlantı hatası oluştu', 'error');
            document.getElementById('avatar-preview').style.opacity = '1';
        });
...
<script src="<?php echo $img_prefix; ?>assets/js/app.js"></script>
<script>
// Initialize App for Navigation
app.isLoggedIn = true;
</script>
<!-- Bottom Navigation -->
