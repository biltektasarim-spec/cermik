...
// Karakaya Barajı özel verisini çek (Menü resmi için)
$stmt_kk = $pdo->prepare("SELECT name, name_en, image_main, slogan, slogan_en FROM places WHERE id = 26 LIMIT 1");
$stmt_kk->execute();
$karakaya_menu = $stmt_kk->fetch();
...
// 3. Özel İlçe Sayfası (Karakaya Barajı)
if ($karakaya_menu) {
    $kk_title = ($is_en) ? ($karakaya_menu['name_en'] ?? $settings['hero_title_en'] ?? 'District Guide') : ($karakaya_menu['name'] ?? $settings['hero_title_tr'] ?? 'Karakaya Barajı');
    $kk_slogan = ($is_en) ? ($karakaya_menu['slogan_en'] ?? $settings['hero_desc_en'] ?? '') : ($karakaya_menu['slogan'] ?? $settings['hero_desc_tr'] ?? '');
    
    $all_banners[] = [
        'type' => 'special',
        'title' => $kk_title,
        'slogan' => $kk_slogan,
        'image' => $karakaya_menu['image_main'] ?? $settings['hero_image'] ?? 'assets/img/categories/kaplica.jpg',
        'url' => 'karakaya_baraji.php',
...
<head>
    ...
    <title><?php echo $settings['site_name'] ?? 'Çüngüş Rehberi'; ?> - ROTAREHBER</title>
...
<body data-page-context="Genel Çüngüş Rehberi" data-district-slug="cungus">
...
<div id="app">
    ...
    <div class="sidebar-menu" id="sidebar-menu">
        ...
        <div class="sidebar-lang-switcher">
            <p style="font-size: 0.75rem; color: var(--text-secondary); text-transform: uppercase; margin-bottom: 10px; letter-spacing: 1px;">Dil / Language</p>
            <div class="lang-btn-row">
                <a href="?lang=tr" class="lang-btn <?php echo ($_SESSION['lang'] ?? 'tr') == 'tr' ? 'active' : ''; ?>">Türkçe</a>
                <a href="?lang=en" class="lang-btn <?php echo ($_SESSION['lang'] ?? 'tr') == 'en' ? 'active' : ''; ?>">English</a>
            </div>
        </div>
    </div>
            ...
            <div style="margin-bottom: 25px;">
                ...
                <?php foreach($all_banners as $b): ?>
                    ...
                    <div class="category-item hero-banner" 
                         ...
                         data-lat="<?php echo $b['lat'] ?? '0'; ?>" 
                         data-lng="<?php echo $b['lng'] ?? '0'; ?>" 
                    ...
                    <div class="category-item hero-banner" onclick="window.location.href='<?php echo $b['url']; ?>'" style="width: 100%; height: 160px; position: relative; overflow: hidden; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.3); margin-bottom: 20px;">
                ...
                <h2 style="margin-bottom: 5px; font-size: 0.9rem; opacity: 0.8; font-weight: 400;"><?php echo htmlspecialchars(($is_en ? ($settings['explore_desc_en'] ?? '') : ($settings['explore_desc'] ?? '')) ?: ($is_en ? 'Explore' : 'Doğanın Kalbi')); ?></h2>
            </div>
