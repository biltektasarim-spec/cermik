...
$district_id = 5; // Çüngüş için sabitlendi
$settings = get_settings($pdo, $district_id);

// Botu çalıştırarak Çüngüş nöbetçi eczaneleri güncelle (2 saatlik cache kontrollü)
// Çüngüş için diyarbakireo.org.tr ilçe kodu arka planda (fetch_pharmacies.php içinde) otomatik eşleşir
require_once __DIR__ . '/../api/fetch_pharmacies.php';
fetchPharmacies($district_id);

// Nöbetçi Eczaneleri Çek
$stmt = $pdo->prepare("SELECT * FROM pharmacies WHERE is_on_duty = 1 AND district_id = ? ORDER BY name ASC");
$stmt->execute([$district_id]);
$pharmacies = $stmt->fetchAll();

// Hastaneleri Çek
$stmt = $pdo->prepare("SELECT * FROM hospitals WHERE district_id = ? ORDER BY name ASC");
$stmt->execute([$district_id]);
...
<head>
    ...
    <title><?php echo __('pharmacy_title'); ?> - <?php echo $settings['site_name'] ?? 'Çüngüş'; ?></title>
...
<body data-page-context="<?php echo __('pharmacy_title'); ?>" data-district-slug="cungus">
...
<div id="app">
    ...
    <?php if (!empty($pharmacies)): ?>
        <div class="animate-in" style="background: linear-gradient(135deg, #e53e3e, #b83232); color: white; padding: 25px; text-align: center; border-radius: 0 0 30px 30px; margin-bottom: 25px; box-shadow: 0 10px 20px rgba(229, 62, 62, 0.2);">
            <div style="font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; opacity: 0.9; margin-bottom: 5px;"><?php echo str_replace('.', $settings['site_name'] ?? 'Çüngüş', __('on_duty_today')); ?></div>
        ...
        <?php if (!empty($pharmacies)): ?>
            ...
            <div id="pharmacy-list" class="menu-list">
                <?php foreach ($pharmacies as $p): ?>
                    <div class="card animate-in menu-item-bg" style="background-image: url('<?php echo resolve_image_path('assets/img/categories/medical.jpg'); ?>');" data-lat="<?php echo $p['lat']; ?>" data-lng="<?php echo $p['lng']; ?>">
        ...
        <?php if (!empty($hospitals)): ?>
            <div id="hospital-list" class="menu-list">
                <?php foreach ($hospitals as $h): ?>
                    <div class="card hospital-card animate-in menu-item-bg" style="background-image: url('<?php echo resolve_image_path($h['image_main'] ?: 'assets/img/categories/medical_bg.jpg'); ?>');" data-lat="<?php echo $h['lat']; ?>" data-lng="<?php echo $h['lng']; ?>">
                        <div class="menu-card-overlay"></div>
                        <div style="position: relative; z-index: 1;">
