...
if (isset($_GET['district_id']) && $_SESSION['admin_role'] === 'SUPER_ADMIN') {
    $edit_district_id = intval($_GET['district_id']);
} else {
    $edit_district_id = $admin_restricted_district > 0 ? $admin_restricted_district : 5;
}
...
if (!$place) {
    ...
    $pdo->prepare("
        INSERT INTO places (
            district_id, name, name_en, category, 
            description, description_en, 
            hastaliklar, hastaliklar_en, 
            slogan, slogan_en, 
            heading_hastaliklar_tr, heading_hastaliklar_en, 
            lat, lng, image_main
        ) VALUES (
            ?, ?, ?, 'HotSpring', 
            ?, ?, 
            'Bilgi girilmedi...', 'No info provided...', 
            '', '', 
            '', '', 
            ?, ?, 'assets/img/categories/kaplica.jpg'
        )
    ")->execute([
        $edit_district_id, 
        $default_name, $default_name . ' (EN)', 
        $default_desc, $default_desc . ' (EN)', 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        header("Location: karakaya_edit.php?district_id={$edit_district_id}&msg=updated");
        exit;
    } catch (Exception $e) {
        ...
        <main class="page-content">
            ...
            <?php if (isset($_GET['msg']) && $_GET['msg'] == 'updated'): ?>
                <div style="background: #e8f5e9; color: #2e7d32; padding: 1rem; border-radius: 8px; margin-bottom: 1rem;">
                    Karakaya Barajı bilgileri başarıyla güncellendi.
                </div>
            <?php endif; ?>
            ...
            <div class="card">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="slogan_en" value="<?php echo htmlspecialchars($place['slogan_en'] ?? ''); ?>" placeholder="Example: Locomotive of National Economy...">
                        <small style="color:#777;">Karakaya Barajı sayfasının ana görseli üzerindeki açıklama yazısı.</small>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                        <div class="form-group">
                            <label>Bilgi Bölüm Başlığı (TR)</label>
                            <input type="text" name="heading_hastaliklar_tr" value="<?php echo htmlspecialchars($place['heading_hastaliklar_tr'] ?? 'Ülke Ekonomisine Katkısı'); ?>" placeholder="Örn: Ülke Ekonomisine Katkısı">
                        </div>
                        <div class="form-group">
                            <label>Bilgi Bölüm Başlığı (EN)</label>
                            <input type="text" name="heading_hastaliklar_en" value="<?php echo htmlspecialchars($place['heading_hastaliklar_en'] ?? 'Contribution to Country Economy'); ?>" placeholder="Example: Contribution to Country Economy">
                        </div>
                    </div>
