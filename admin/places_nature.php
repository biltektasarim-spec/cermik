...
if (isset($_POST['save_titles'])) {
    $tr = $_POST['title_tr'];
    $en = $_POST['title_en'];
    $pdo->prepare("INSERT INTO settings (name, value, district_id) VALUES ('menu_nature_tr', ?, ?) ON DUPLICATE KEY UPDATE value = ?")->execute([$tr, $admin_district_id, $tr]);
    $pdo->prepare("INSERT INTO settings (name, value, district_id) VALUES ('menu_nature_en', ?, ?) ON DUPLICATE KEY UPDATE value = ?")->execute([$en, $admin_district_id, $en]);
    ...
    if (isset($_FILES['cat_image']) && $_FILES['cat_image']['error'] == 0) {
        ...
        $new_filename = 'nature_' . $admin_district_id . '_' . time() . '.' . $file_ext;
        if (move_uploaded_file($_FILES['cat_image']['tmp_name'], $target_dir . $new_filename)) {
            $img_path = 'uploads/categories/' . $new_filename;
            $pdo->prepare("INSERT INTO settings (name, value, district_id) VALUES ('menu_nature_img', ?, ?) ON DUPLICATE KEY UPDATE value = ?")->execute([$img_path, $admin_district_id, $img_path]);
        }
    }
    header("Location: places_nature.php?msg=saved");
    exit;
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt->execute([$id]);
    header("Location: places_nature.php?msg=deleted");
    exit;
}

$stmt_s = $pdo->prepare("SELECT value FROM settings WHERE name = 'menu_nature_tr' AND (district_id = ? OR district_id = 0) ORDER BY district_id DESC LIMIT 1");
$stmt_s->execute([$admin_district_id]);
$val_tr = $stmt_s->fetchColumn() ?: 'Doğal Güzellikler';

$stmt_en = $pdo->prepare("SELECT value FROM settings WHERE name = 'menu_nature_en' AND (district_id = ? OR district_id = 0) ORDER BY district_id DESC LIMIT 1");
$stmt_en->execute([$admin_district_id]);
$val_en = $stmt_en->fetchColumn() ?: 'Nature and Parks';

$stmt_img = $pdo->prepare("SELECT value FROM settings WHERE name = 'menu_nature_img' AND (district_id = ? OR district_id = 0) ORDER BY district_id DESC LIMIT 1");
$stmt_img->execute([$admin_district_id]);
$cat_img = $stmt_img->fetchColumn() ?: 'assets/img/categories/nature.jpg';

// Doğa & Parkları çek
$places = $pdo->query("SELECT p.*, d.name as district_name FROM places p LEFT JOIN districts d ON p.district_id=d.id WHERE p.category='Nature' AND ($admin_filter) ORDER BY p.created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="tr">
<head>
    ...
    <title>Doğa & Parklar - Yönetim</title>
        ...
        <header style="display: flex; align-items: center; gap: 20px;">
            <div style="display: flex; align-items: center; gap: 15px; flex: 1;">
                ...
                <div class="header-left">
                    <h1>Doğa & Park Yönetimi</h1>
                    <p style="color: var(--text-muted);"><?php echo count($places); ?> mekan kayıtlı.</p>
                </div>
            </div>
            <div class="header-right">
                <a href="place_add.php?cat=Nature" class="btn btn-primary"><i class="fa-solid fa-plus"></i> Yeni Doğa Mekanı Ekle</a>
            </div>
        </header>
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display: flex; gap: 5px;">
                                    ...
                                    <a href="places_nature.php?delete=<?php echo $p['id']; ?>" class="btn" onclick="return confirm('Silmek istediğinize emin misiniz?')" style="padding: 0.4rem 0.8rem; border: 1px solid #ddd; color: #e74c3c;">
